...
let defaultWingsuitAutoTurnRatiosResolved = WingsuitAutoTurnRatios(
    turn1: 0.75,
    turn2: 0.3125
)

struct ResolvedWingsuitAutoTuning {
    ...
    let maxDeployRadiusFt: Double
    let maxFirstLegTrackDeltaDeg: Double
    let minDeployRadiusFt: Double
    let refinementIterations: Double
    let exitOnJumpRunToleranceFt: Double
}

let defaultWingsuitAutoTuningResolved = ResolvedWingsuitAutoTuning(
...
func vectorToHeadingDeg(_ vector: Vec2) -> Double {
...
func localPointFromGeoPoint(reference: GeoPoint, point: GeoPoint) -> LocalPoint {
...
func geoPointFromLocal(reference: GeoPoint, point: LocalPoint) -> GeoPoint {
...
func pointToUnitVector(headingDeg: Double) -> LocalPoint {
    let unit = headingToUnitVector(headingDeg)
    return LocalPoint(eastFt: unit.east, northFt: unit.north)
}

func localPointMagnitude(_ point: LocalPoint) -> Double {
    hypot(point.eastFt, point.northFt)
}

func localPointDifference(_ a: LocalPoint, _ b: LocalPoint) -> LocalPoint {
    LocalPoint(eastFt: a.eastFt - b.eastFt, northFt: a.northFt - b.northFt)
}

func localPointAdd(_ a: LocalPoint, _ b: LocalPoint) -> LocalPoint {
    LocalPoint(eastFt: a.eastFt + b.eastFt, northFt: a.northFt + b.northFt)
}

func scaleLocalPoint(_ point: LocalPoint, _ scalar: Double) -> LocalPoint {
    LocalPoint(eastFt: point.eastFt * scalar, northFt: point.northFt * scalar)
}

func dotLocalPoint(_ a: LocalPoint, _ b: LocalPoint) -> Double {
    a.eastFt * b.eastFt + a.northFt * b.northFt
}

func crossLocalPoint(_ a: LocalPoint, _ b: LocalPoint) -> Double {
    a.eastFt * b.northFt - a.northFt * b.eastFt
}

func normalizeLocalPoint(_ point: LocalPoint) -> LocalPoint {
...
func buildJumpRunFrame(landingPoint: GeoPoint, jumpRun: JumpRunLine) -> JumpRunFrame? {
...
func signedCrossTrackDistanceFt(frame: JumpRunFrame, point: LocalPoint) -> Double {
    dotLocalPoint(localPointDifference(point, frame.start), frame.leftUnit)
}

func alongJumpRunDistanceFt(frame: JumpRunFrame, point: LocalPoint) -> Double {
    dotLocalPoint(localPointDifference(point, frame.start), frame.unit)
}

func circularDifferenceDeg(_ a: Double, _ b: Double) -> Double {
    let delta = abs(normalizeHeading(a) - normalizeHeading(b))
    return min(delta, 360 - delta)
}

func resolveTurnRatios(_ turnRatios: WingsuitAutoTurnRatios?) -> WingsuitAutoTurnRatios {
...
func resolveWingsuitAutoTuning(_ tuning: WingsuitAutoTuning?) -> ResolvedWingsuitAutoTuning {
    ResolvedWingsuitAutoTuning(
        ...
        maxDeployRadiusFt: tuning?.maxDeployRadiusFt ?? defaultWingsuitAutoTuningResolved.maxDeployRadiusFt,
        maxFirstLegTrackDeltaDeg: tuning?.maxFirstLegTrackDeltaDeg ?? defaultWingsuitAutoTuningResolved.maxFirstLegTrackDeltaDeg,
...
func deriveTurnHeightsFt(input: WingsuitAutoInput, ratios: WingsuitAutoTurnRatios) -> [Double] {
...
func buildAutoGatesFt(input: WingsuitAutoInput, ratios: WingsuitAutoTurnRatios) -> [Double] {
...
func isFiniteGeoPoint(_ point: GeoPoint) -> Bool {
    point.lat.isFinite &&
        point.lng.isFinite &&
...
public func validateWingsuitAutoInput(_ input: WingsuitAutoInput) -> ValidationResult {
    ...
    let directionMode = input.jumpRun.directionMode ?? .auto
    let constraintMode = input.jumpRun.constraintMode ?? .none
    let placementMode = resolveJumpRunPlacementMode(input.jumpRun.placementMode)
    if directionMode == .manual && !(input.jumpRun.manualHeadingDeg?.isFinite ?? false) {
        errors.append("Manual jump-run direction requires a finite heading.")
    }
    if constraintMode == .reciprocal && !(input.jumpRun.constraintHeadingDeg?.isFinite ?? false) {
        errors.append("Reciprocal jump-run constraint requires a finite runway heading.")
    }
    if placementMode == .distance {
        let distanceOffsetFt = resolveDistanceOffsetFt(input)
        let postTurnFt = resolveDistancePostTurnFt(input)
        if !distanceOffsetFt.isFinite || distanceOffsetFt <= 0 {
            errors.append("Distance-mode offsite distance must be finite and positive.")
        }
        if !postTurnFt.isFinite || postTurnFt < 0 {
            errors.append("Distance-mode post-turn offset must be finite and nonnegative.")
        }
    }
    ...
    if !assumptions.planeAirspeedKt.isFinite ||
        !assumptions.groupSeparationFt.isFinite ||
        !assumptions.slickDeployHeightFt.isFinite ||
        !assumptions.slickFallRateFps.isFinite ||
        !assumptions.slickReturnRadiusFt.isFinite {
        errors.append("Jump-run assumptions must be finite when provided.")
    } else {
        if assumptions.planeAirspeedKt <= 0 {
            errors.append("Plane airspeed must be positive.")
        }
        if assumptions.groupCount < 1 {
            errors.append("Wingsuit exit group must be an integer of at least 1.")
        }
        if assumptions.groupSeparationFt <= 0 {
            errors.append("Group separation must be positive.")
        }
        if !(assumptions.slickDeployHeightFt > 0 && assumptions.slickDeployHeightFt < input.exitHeightFt) {
            errors.append("Slick deploy height must be above 0 ft and below exit height.")
        }
        if assumptions.slickFallRateFps <= 0 {
            errors.append("Slick fall rate must be positive.")
        }
        if assumptions.slickReturnRadiusFt <= 0 {
            errors.append("Slick return radius must be positive.")
        }
    }
    ...
    let highestWind = input.winds.reduce(-Double.greatestFiniteMagnitude) { max($0, $1.altitudeFt) }
    ...
    if lowestWind.isFinite && lowestWind > Swift.min(input.deployHeightFt, assumptions.slickDeployHeightFt) {
        warnings.append("Lowest wind layer is above the lowest modeled altitude; low winds will be extrapolated.")
    }
...
// The forward-parametric wingsuit auto solver is in WingsuitAutoSolver.swift
